...
module.exports = {
  ...
  callback: async (client, interaction) => {
    ...
    if (!token) {
      ...
      console.log(`[${getTimestamp()}][ACCESS DENIED]: ${interaction.user.tag} tried to run the command /gpublish but they do not have access to this command.`);
      return;
    }
    ...
    const CreateGame = async (csrf) => {
      try {
        ...
        console.log(`[${getTimestamp()}][GROUP PUBLISHER]: Game created! (gameId: ${response.data.rootPlaceId})`);
        return response;
      } catch (error) {
    ...
    const uploadGame = async (csrf, placeId, success, failed, loading) => {
      const response = await axios.post(`https://data.roblox.com/Data/Upload.ashx?assetid=${placeId}`, fs.readFileSync(gameFile), {
        ...
        console.log(`[${getTimestamp()}][GROUP PUBLISHER]: Game uploaded! (gameId: ${placeId})`);
    ...
    const sendRequests = async () => {
      try {
        ...
        .setTitle(':tada: Your game is now published in your group!')
    ...
    // Final requests and interactions
      sendRequests();
      console.log(`[${getTimestamp()}][GROUP PUBLISHER]: ${interaction.user.tag} has published a game using our theme ${interaction.options.getString('games')}!`)
    } catch (error) {
  ...
  name: 'gpublish',
    ...
    options: [
      ...
      {
        name: 'group-id',
        description: 'ID of the group you want to use.',
        type: ApplicationCommandOptionType.String,
        required: true,
      },
      {
